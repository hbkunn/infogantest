...
class FasterRCNN(nn.Module):
    ...
    def interpret_faster_rcnn(self, cls_prob, bbox_pred, rois, im_info, im_shape, nms=True, clip=False, min_score=0.0):
        # find class
        ...
        if nms and pred_boxes.shape[0] > 0:
            pred_boxes, scores, inds = nms_detections(pred_boxes, scores, 0.4, inds=inds)
    ...
    def get_image_blob(self, im):
        ...
        blob = im_list_to_blob(processed_ims)
        print (blob.shape, np.array(im_scale_factors).shape)
        return blob, np.array(im_scale_factors)
