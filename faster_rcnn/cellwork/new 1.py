...
for i in imageList[0:-1]:
    ...
    for hight in range(0,6):
        for width in range(0,6):
			img=img[hignt*200:hight*200+200,width*200:width*200+200,::-1]
			label=label[hignt*200:hight*200+200,width*200:width*200+200,::-1]
            data_dict = {}
            lst = []
            for x in range(hight*200,hight*200+200):
                for y in range(width*200,width*200+200):
                    if label[x,y] == 255:
                        lst.append([x-hight*200,y-width*200])
...
root_data = '/home/tangye/faster_rcnn_pytorch/data/BM_GRAZ/augmentation/data/'
root_label = '/home/tangye/faster_rcnn_pytorch/data/BM_GRAZ/augmentation/label/'
imageList = os.listdir(root_data)
for i in imageList[0:-1]:
    img = cv2.imread(root_data + i)    
    label = cv2.imread(root_label + i, 2)
